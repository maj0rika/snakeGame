<template>
<div>
    <canvas id="스네이크" ref="스네이크" :width="boardSizePx" :height="boardSizePx"> </canvas>
</div>
</template>
    ...
    methods: {
        ...
        pressKey(event){
            ...
            if(!newDirection){
             ...
             if (Math.abs(newDirection.keyCode - this.direction.keyCode) !== 2) {//180도 회전 막기
        ...
        applePx(){
            if(!this.targetPx){
                ...
                while (this.countPixelInSnake(targetPx) > 0) {// 뱀몸속에 먹이 생기기 막기
